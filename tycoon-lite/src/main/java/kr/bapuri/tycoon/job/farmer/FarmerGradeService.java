...
/**
 * FarmerGradeService - 농부 등급 관리 서비스
 * 
 * Phase 4.C 구현:
 * - AbstractJobGradeService 상속
 * - 등급별 승급 조건 (레벨, BD, 총 판매액)
 * - FarmerConfig 기반 조건 로드
 */
public class FarmerGradeService extends AbstractJobGradeService {
    
    private final FarmerConfig config;
    
    public FarmerGradeService(JavaPlugin plugin,
                              PlayerDataManager dataManager,
                              EconomyService economyService,
                              FarmerConfig config) {
        super(plugin, dataManager, economyService, JobType.FARMER);
        this.config = config;
    }
    ...
    protected long getRequiredBD(JobGrade grade) {
        FarmerConfig.GradeRequirement req = config.getGradeRequirement(grade);
        if (req == null) {
            // 기본값 (농부: T2=Lv25, T3=Lv50, T4=Lv90)
            return switch (grade) {
                case GRADE_1 -> 0;
                case GRADE_2 -> 50000;
                case GRADE_3 -> 200000;
                case GRADE_4 -> 1000000;
            };
        }
    ...
    protected String checkAdditionalRequirements(Player player, PlayerTycoonData data, JobGrade targetGrade) {
        FarmerConfig.GradeRequirement req = config.getGradeRequirement(targetGrade);
        if (req == null) return null;
        
        // [Phase 4.D] 총 수확량 조건
        if (req.requiredTotalHarvested > 0) {
            long totalHarvested = data.getTotalHarvested();
            if (totalHarvested < req.requiredTotalHarvested) {
                return String.format("§c총 수확량이 부족합니다. §7(현재 %,d / 필요 %,d개)",
                        totalHarvested, req.requiredTotalHarvested);
            }
        }
        
        // [Phase 4.D] 작물 판매액 조건
        if (req.requiredTotalSoldAmount > 0) {
            long farmerSales = data.getTotalFarmerSales();
            if (farmerSales < req.requiredTotalSoldAmount) {
                return String.format("§c작물 판매액이 부족합니다. §7(현재 %,d / 필요 %,d BD)",
                        farmerSales, req.requiredTotalSoldAmount);
            }
        }
    ...
    protected String getAdditionalRequirementsString(Player player, JobGrade targetGrade) {
        ...
        // [Phase 4.D] 총 수확량 조건
        if (req.requiredTotalHarvested > 0) {
            long totalHarvested = data != null ? data.getTotalHarvested() : 0;
            String status = totalHarvested >= req.requiredTotalHarvested ? "§a✓" : "§c✗";
            sb.append(String.format("%s 총 수확량 %,d개 이상 (현재: %,d)\n",
                    status, req.requiredTotalHarvested, totalHarvested));
        }
        
        // [Phase 4.D] 작물 판매액 조건
        if (req.requiredTotalSoldAmount > 0) {
            long farmerSales = data != null ? data.getTotalFarmerSales() : 0;
            String status = farmerSales >= req.requiredTotalSoldAmount ? "§a✓" : "§c✗";
            sb.append(String.format("%s 작물 판매액 %,d BD 이상 (현재: %,d)\n",
                    status, req.requiredTotalSoldAmount, farmerSales));
        }
    ...
    protected void onPromote(Player player, JobGrade oldGrade, JobGrade newGrade) {
        ...
        String bonusInfo = switch (newGrade) {
            case GRADE_2 -> "§7보너스: 수확량 +5%";
            case GRADE_3 -> "§7보너스: 수확량 +10%, 프라임 확률 +3%";
            case GRADE_4 -> "§7보너스: 수확량 +15%, 프라임 확률 +5%, 트로피 확률 +2%";
            default -> null;
        };
    ...
    public void reloadConfig() {
        ...
        logger.info("[FarmerGradeService] 설정 리로드 완료");
    }
    ...
    public String getProgressString(Player player) {
        ...
        // 판매액 진행률
        FarmerConfig.GradeRequirement req = config.getGradeRequirement(nextGrade);
